...
-- Add missing columns to user_profiles safely
DO $$
BEGIN
...
-- Fix clients table - ensure it exists with proper structure
DO $$
BEGIN
...
-- Create document_requests table (depends on clients)
...
-- Fix documents table - ensure proper structure
DO $$
BEGIN
  -- Drop and recreate documents table if it has wrong structure
...
-- Create projects table (depends on clients)
...
-- Add project_id to document_requests if missing
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'document_requests' AND column_name = 'project_id') THEN
    ALTER TABLE document_requests ADD COLUMN project_id uuid REFERENCES projects(id) ON DELETE SET NULL;
  END IF;
END $$;
...
-- Fix custom_services table if it exists with wrong structure
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'custom_services') THEN
    -- Check if it has the right columns
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'custom_services' AND column_name = 'billing_type') THEN
      DROP TABLE custom_services CASCADE;
    END IF;
  END IF;
  
  -- Create custom_services table with proper structure
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'custom_services') THEN
...
-- Create service_orders table (depends on clients and custom_services)
...
-- Create transactions table (depends on service_orders and clients)
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'notifications') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'notifications' AND column_name = 'title_i18n') THEN
...
-- Create tasks table if not exists (depends on clients and projects)
...
CREATE INDEX IF NOT EXISTS idx_blog_posts_country ON blog_posts(country_id);
CREATE INDEX IF NOT EXISTS idx_blog_posts_published ON blog_posts(is_published);
CREATE INDEX IF NOT EXISTS idx_blog_posts_featured ON blog_posts(is_featured);

CREATE INDEX IF NOT EXISTS idx_faqs_consultant ON faqs(consultant_id);
CREATE INDEX IF NOT EXISTS idx_faqs_country ON faqs(country_id);
...
CREATE POLICY "Consultants manage own tasks" ON tasks
  FOR ALL TO authenticated
  USING (consultant_id = auth.uid())
  WITH CHECK (consultant_id = auth.uid());

CREATE POLICY "Clients read visible tasks" ON tasks
  FOR SELECT TO authenticated
  USING (
...
CREATE POLICY "Consultants manage own time entries" ON time_entries
  FOR ALL TO authenticated
  USING (consultant_id = auth.uid())
  WITH CHECK (consultant_id = auth.uid());
