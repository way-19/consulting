/*
  # Complete Professional Dashboard System - Clean Rebuild

  1. Clean Slate Approach
    - Drop all existing problematic tables
    - Rebuild with proper dependency order
    - Fix all foreign key relationships
    - Eliminate variable ambiguity issues
  ...
  4. Sample Data
    - Test users and relationships
    - Sample projects and tasks
    - Document requests and notifications
*/

-- Drop all existing tables to start clean (preserve core tables)
...
-- 6. Document requests table (depends on clients and projects)
...
-- Sample data for testing (using explicit variable names to avoid ambiguity)
DO $$
DECLARE
  v_admin_id uuid;
...
BEGIN
  ...
  SELECT id INTO v_admin_id FROM user_profiles WHERE email = 'admin@consulting19.com';
  SELECT id INTO v_consultant_id FROM user_profiles WHERE email = 'giorgi.meskhi@consulting19.com';
  SELECT id INTO v_client_id FROM user_profiles WHERE email = 'client@consulting19.com';
  SELECT id INTO v_georgia_id FROM countries WHERE code = 'ge' OR name ILIKE '%georgia%' LIMIT 1;
  SELECT id INTO v_category_id FROM document_categories WHERE slug = 'identity' LIMIT 1;
